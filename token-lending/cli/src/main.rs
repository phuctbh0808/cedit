...
fn command_withdraw_collateral(
    ...
    let relend_state = RelendState::new(
        config.lending_program_id,
        obligation_pubkey,
        &config.rpc_client,
    );

    let withdraw_reserve = relend_state
    ...
    let instructions = relend_state.withdraw(&withdraw_reserve_pubkey, collateral_amount);
    let recent_blockhash = config.rpc_client.get_latest_blockhash()?;
    let transaction = Transaction::new(
        &vec![config.fee_payer.as_ref()],
        Message::new_with_blockhash(
...
fn command_update_reserve(
    ...
    let mut new_pyth_product_pubkey = relend_sdk::NULL_PUBKEY;
    if pyth_price_pubkey.is_some() {
        ...
        reserve.liquidity.oracle_pubkey = pyth_price_pubkey.unwrap();
    ...
    if switchboard_feed_pubkey.is_some() {
        ...
        reserve.liquidity.second_oracle_pubkey = switchboard_feed_pubkey.unwrap();
    }
